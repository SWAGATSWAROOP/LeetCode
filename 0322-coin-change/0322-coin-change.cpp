...
public:
    int possible(vector<int>& coins, int amount,int index,int sum,vector<vector<int>>& v){
        ...
        if(index == coins.size() || sum > amount || coins[index] >1e4)return 1e7;
        if(v[index][sum] != -1)return v[index][sum];
        int mn = 1e5;
        mn = 1+min(possible(coins,amount,index+1,sum+coins[index],v),possible(coins,amount,index,sum+coins[index],v)); //Doing This error adding 1 before
